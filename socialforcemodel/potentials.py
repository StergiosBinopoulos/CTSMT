...
from socialforcemodel import stateutils
...
class PedSpacePotential(object):
    ...
    def r_aB(self, state):
        ...
        closest_points = np.swapaxes(
            np.stack([B[i] for B, i in zip(self.space, closest_i)]), 0, 1)  # index order: pedestrian, boundary, coordinates
